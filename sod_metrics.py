...
#                     2
#                     * (combination[0] * combination[1])
#                     / (combination[0] ** 2 + combination[1] ** 2 + 1e-20)
#                 )
#                 enhanced_matrix_value = (align_matrix_value + 1) ** 2 / 4
#                 results_parts.append(enhanced_matrix_value * part_numel)
#             enhanced_matrix_sum = sum(results_parts)
#
#         em = enhanced_matrix_sum / (self.gt_size - 1 + 1e-20)
#         return em
#
#     def cal_em_with_cumsumhistogram(self, pred: np.ndarray, gt: np.ndarray) -> np.ndarray:
#         """
...
#         self.beta2 = 0.3
#         self.avg_f=0.0
#         self.prec_avg = torch.zeros(255)
#         self.recall_avg = torch.zeros(255)
#         self.img_num=0
#         # self.score=0.0
#
#     def step(self, pred: np.ndarray, gt: np.ndarray):
...
#             f_score = 0
#         else:
#             f_score = self.cal_fm(pred, gt)
#             self.avg_f+=f_score
#             self.img_num+=1
#
#
#     def cal_fm(self, pred: np.ndarray, gt: np.ndarray) -> float:
#
...
#         f_score = (1 + self.beta2) * prec * recall / (self.beta2 * prec + recall + 1e-20)
...
#     def get_results(self) -> dict:
#
#         score = self.avg_f / self.img_num
#         return dict(meanF=score.mean().item())
